...
const vscode = {
  ...
  window: {
    ...
    showInformationMessage: (...args: unknown[]) => { console.log('[INFO]', ...args); return Promise.resolve(); },
    showWarningMessage: (...args: unknown[]) => { console.log('[WARN]', ...args); return Promise.resolve(); },
    showErrorMessage: (...args: unknown[]) => { console.log('[ERROR]', ...args); return Promise.resolve(); },
    createOutputChannel: () => ({
      appendLine: (msg: string) => console.log('[OUTPUT]', msg),
      show: () => {}
    })
  },
  Uri: {
...
async function testPipeline() {
  ...
  try {
    new ContextService(storage!, geminiService!); // Initialize to verify it works
  ...
  try {
    ...
    if (idleResult) {
      ...
      const session = await storage!.createSession(
        idleResult.summary,
        vscode.workspace.name || 'test-project'
      );
      ...
      const similarSessions = await storage!.getSimilarSessions(
        'working on user profile component',
        3
      );
