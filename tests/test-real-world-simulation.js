    ...
    return {
        ...
        window: {
            ...
            createOutputChannel: (name) => ({
                name,
                append: (msg) => console.log(`[${name}]`, msg),
...
const ContextIngestionService_1 = require("./src/services/ingestion/ContextIngestionService");
 ...
 */
function getRecentFileEdits() {
    try {
        // Get last 10 commits with file changes
        const gitLog = (0, child_process_1.execSync)('git log --name-status --pretty=format:"%H|%an|%ae|%at|%s" -10', { encoding: 'utf8', cwd: process.cwd() });
...
async function simulateRealSession() {
    ...
    const contextService = new ContextService_1.ContextService(storage, geminiService);
    ...
    const ingestionService = new ContextIngestionService_1.ContextIngestionService(storage, contextService, sessionManager);
    ...
    await ingestionService.initialize(mockContext, outputChannel);
    ...
    const autonomousAgent = new AutonomousAgent_1.AutonomousAgent(gitService, geminiService, contextService);
    log.success('Autonomous Agent ready');
    const idleService = new idle_service_1.IdleService(storage, { thresholdMs: 15000 }, geminiService);
...
//# sourceMappingURL=test-real-world-simulation.js.map