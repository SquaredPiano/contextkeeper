...
const createUltraVSCodeMock = () => {
    ...
    return {
        ...
        window: {
            ...
            showInformationMessage: async (...args) => { console.log('[INFO]', ...args); },
            showWarningMessage: async (...args) => { console.log('[WARN]', ...args); },
            showErrorMessage: async (...args) => { console.log('[ERROR]', ...args); },
        ...
        Uri: {
            file: (filePath) => ({ fsPath: filePath, path: filePath, scheme: 'file' })
        },
        Position: class {
            line;
            character;
...
const log = {
    ...
    metric: (name, value) => console.log(`  ${name.padEnd(40)}: \x1b[36m${value}\x1b[0m`)
};
const metrics = [];
async function extremeValidation() {
    ...
    const allFiles = (0, child_process_1.execSync)('find src -name "*.ts" -not -name "*.test.ts" | head -35', { encoding: 'utf8' })
        .split('\n')
        .filter(Boolean);
...
//# sourceMappingURL=test-extreme-validation.js.map